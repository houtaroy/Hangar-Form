import { has, get, pick, keys, values } from 'lodash';
...
class ApiOptionsParser extends BaseOptionsParser {
  ...
  parseApiResultByOptions(apiResult, options) {
    const result = [];
    apiResult[0].items.forEach(data => {
...
class DynamicOptionsParser extends BaseOptionsParser {
  ...
  parse(element, context) {
    const { optionsConfig: config, key: elementKey } = element;
    const { name: dynamicName } = config;
    has(context.optionsDynamic, dynamicName)
      ? context.optionsDynamic[dynamicName].push(elementKey)
      : this._ref(elementKey, dynamicName, context);
  }
  _ref(elementKey, dynamicName, context) {
    context.optionsDynamic[dynamicName] = [elementKey];
    let data = context[dynamicName];
    Object.defineProperty(context, dynamicName, {
      get: function() {
        return data;
      },
      set: function(newValue) {
        data = newValue;
        context.optionsDynamic[dynamicName].forEach(key => {
          context.optionsMap[key] = newValue;
        });
      }
    });
  }
